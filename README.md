...
模块默认每天 `08:30` 自动签到。在 Surge iOS 的模块列表中点击“一点万象自动签到”右侧菜单，选择“编辑参数”：

- `HOUR`：小时，使用 24 小时制，范围 `0-23`。
- `MINUTE`：分钟，范围 `0-59`。

例如每天 `09:05` 执行，设置 `HOUR=9`、`MINUTE=5`。

从不带参数的旧版本升级时，仅使用“现在更新”可能不会重建参数表。请删除旧模块并通过安装 URL 重新安装一次，之后菜单中会出现“编辑参数”。

### 验证定时触发

可临时安装 `MixcSignIn-Test.sgmodule`。它每 5 分钟运行一次，并在今日已经签到时发送“定时测试已触发”通知。确认收到通知后应立即停用或删除测试模块，避免不必要的频繁唤醒；正式模块仍保持每天 `08:30`。

定时任务完成后会将结果保存到 Surge `$persistentStore`。再次进入策略组页面时，面板会自动读取并显示这份结果，并更新“面板刷新”时间；这个过程不会发起签到请求。只有点击面板右侧刷新按钮时，才会执行一次手动签到检查。
...
- 默认每天 08:30 自动执行，可通过模块的“编辑参数”修改。
